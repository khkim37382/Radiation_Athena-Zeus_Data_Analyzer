...
ATHENA_NUM_SRS = len(ATHENA_FF_PER_SR)  
...
def build_summary_excel(folder: str, out_xlsx: str, sr_count: int = ATHENA_NUM_SRS) -> None:
    ...
    for i in range(sr_count):
        ...
        ws.cell(row=table_start_row + 1 + i, column=2, value=ATHENA_NUM_BLACKBOXES).alignment = center
    ...
    for idx, fpath in enumerate(seu_files):
        ...
        for i in range(sr_count):
            ...
            total_ffs = ATHENA_NUM_BLACKBOXES * ATHENA_FF_PER_SR[i]
            ...
            # FIT = CrossSection * 1e15 * FIT_FACTOR
